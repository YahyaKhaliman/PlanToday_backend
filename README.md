...
2. **ACH Database (`kpi`)** - Config: [config/dbAch.js](file:///d:/Coding/PlanToday-backend/config/dbAch.js)
   - Digunakan untuk mengelola data visual pencapaian sales, target omset bulanan/tahunan, serta data realisasi performa sales.
3. **Penawaran Database (`kencanaprint`)** - Config: [config/dbPenawaran.js](file:///d:/Coding/PlanToday-backend/config/dbPenawaran.js)
...
npm install
```
*Atau menggunakan perintah npm ci untuk instalasi bersih berbasis package-lock.json:*
```bash
npm ci
```
